  ...
  extends ConstraintWidgetContainer
{
  ...
  private void setChildrenConnections()
  {
    ...
    while (i < k)
    {
      ...
      if ((localObject instanceof Guideline)) {
        localConstraintWidget1.getAnchor(ConstraintAnchor.Type.BOTTOM).connect(((ConstraintWidget)localObject).getAnchor(ConstraintAnchor.Type.TOP), mPadding);
      } else {
  ...
  public void addToSolver(LinearSystem paramLinearSystem, int paramInt)
  {
    ...
    if (paramLinearSystem == mSystem)
    {
      ...
      while (j < m)
      {
        ((ConstraintWidget)mChildren.get(j)).addToSolver(paramLinearSystem, paramInt);
        j += 1;
      }
    }
  }
  ...
  public void updateFromSolver(LinearSystem paramLinearSystem, int paramInt)
  {
    ...
    if (paramLinearSystem == mSystem)
    {
      ...
      while (i < k)
      {
        ((Guideline)mVerticalGuidelines.get(i)).updateFromSolver(paramLinearSystem, paramInt);
        i += 1;
      }
      k = mHorizontalGuidelines.size();
      i = j;
      while (i < k)
      {
        ((Guideline)mHorizontalGuidelines.get(i)).updateFromSolver(paramLinearSystem, paramInt);
        i += 1;
      }
    }
  }
