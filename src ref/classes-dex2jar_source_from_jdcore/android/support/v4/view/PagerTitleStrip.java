  ...
  extends ViewGroup
{
  ...
  void updateTextPositions(int paramInt, float paramFloat, boolean paramBoolean)
  {
    ...
    mCurrText.layout(i3, j, i4, mCurrText.getMeasuredHeight() + j);
    j = Math.min(i2, i3 - mScaledTextSpacing - m);
    mPrevText.layout(j, paramInt, m + j, mPrevText.getMeasuredHeight() + paramInt);
    paramInt = Math.max(n - i1 - k, i4 + mScaledTextSpacing);
    mNextText.layout(paramInt, i, paramInt + k, mNextText.getMeasuredHeight() + i);
