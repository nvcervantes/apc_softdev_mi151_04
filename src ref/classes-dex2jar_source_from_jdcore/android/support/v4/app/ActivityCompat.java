  ...
  public static void requestPermissions(@NonNull final Activity paramActivity, @NonNull String[] paramArrayOfString, @IntRange(from=0L) final int paramInt)
  {
    ...
    if ((paramActivity instanceof OnRequestPermissionsResultCallback)) {
      new Handler(Looper.getMainLooper()).post(new Runnable()
      {
        public void run()
        {
          Object localObject = val$permissions;
          int i = 0;
          ...
          while (i < j)
          {
            localObject[i] = localPackageManager.checkPermission(val$permissions[i], str);
            i += 1;
          }
          ((ActivityCompat.OnRequestPermissionsResultCallback)paramActivity).onRequestPermissionsResult(paramInt, val$permissions, (int[])localObject);
        }
      });
    }
  }
