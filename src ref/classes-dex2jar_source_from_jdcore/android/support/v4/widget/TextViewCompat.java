    ...
    extends TextViewCompat.TextViewCompatApi23Impl
  {
    ...
    public void setCustomSelectionActionModeCallback(final TextView paramTextView, final ActionMode.Callback paramCallback)
    {
      ...
      paramTextView.setCustomSelectionActionModeCallback(new ActionMode.Callback()
      {
        ...
        {
          boolean bool2 = paramAnonymousContext.getPackageName().equals(activityInfo.packageName);
          boolean bool1 = true;
          if (bool2) {
            return true;
          }
          ...
          {
            if (paramAnonymousContext.checkSelfPermission(activityInfo.permission) == 0) {
              return true;
            }
            bool1 = false;
          }
        ...
        private void recomputeProcessTextMenuItems(Menu paramAnonymousMenu)
        {
          ...
          try
          {
            ...
            mMenuBuilderRemoveItemAtMethod = mMenuBuilderClass.getDeclaredMethod("removeItemAt", new Class[] { Integer.TYPE });
          ...
          catch (ClassNotFoundException|NoSuchMethodException localClassNotFoundException)
          {
            for (;;)
            {
              try
              {
                if ((mCanUseMenuBuilderReferences) && (mMenuBuilderClass.isInstance(paramAnonymousMenu))) {
                ...
                if (i < ((List)localObject1).size())
                {
                  ...
                  paramAnonymousMenu.add(0, 0, 100 + i, ((ResolveInfo)localObject2).loadLabel(localPackageManager)).setIntent(createProcessTextIntentForResolveInfo((ResolveInfo)localObject2, paramTextView)).setShowAsAction(1);
                  i += 1;
                  continue;
                }
                return;
